...
function renderSystemPanel(x: number, y: number, width: number, height: number, rotate: number, dominant: boolean, stroke: string, fill: string) {
  return `
...
function renderHelperNode(x: number, y: number, size: number, kind: "spark" | "plus", fill: string, halo: string) {
  return `
...
function renderPressureRings(count: number, color: string) {
  ...
  return rings.slice(0, count).map((ring, index) => `<ellipse cx="${ring.x}" cy="${ring.y}" rx="${ring.rx}" ry="${ring.ry}" transform="rotate(${ring.rotate} ${ring.x} ${ring.y})" stroke="${color}" stroke-opacity="${0.1 + index * 0.06}" stroke-width="2" stroke-dasharray="10 12" />`).join("");
}
