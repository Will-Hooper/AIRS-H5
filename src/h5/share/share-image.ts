...
export async function renderOccupationShareImage({
  ...
  const summary = (language === "zh" ? occupation.summaryZh || occupation.summary : occupation.summary).trim() || copy.noReading;
  const shareInterpretation = `${mascotCopy.riskHint} ${summary}`;
  ...
  const mascotImage = await loadImage(renderMascotSceneDataUrl({ score, language, width: CONTENT_WIDTH, height: heroHeight }));
  ...
  drawMetricCard(context, CONTENT_X, metricY, 434, metricHeight, copy.currentAirsLabel, formatNumber(score, 1, language), mascotCopy.badge, scoreBand.palette.glow);
  drawMetricCard(context, 562, metricY, 434, metricHeight, copy.globalAverageLabel, formatNumber(averageAirs, 1, language), language === "zh" ? "用于和全体职业的当前均值做对照。" : "Context against the current all-occupation average.", scoreBand.palette.helper);
  ...
  const summaryCard = { x: CONTENT_X, y: insightTitleY + 28, width: CONTENT_WIDTH, height: 128 + summaryLayout.lines.length * summaryLayout.lineHeight };
  ...
  drawLines(context, summaryLayout.lines, summaryCard.x + summaryPaddingX, summaryCard.y + 96, summaryLayout.lineHeight);

  const qrDataUrl = await QRCode.toDataURL(siteUrl, {
    margin: 1,
    width: 240,
    errorCorrectionLevel: "H",
    color: { dark: "#0a1220", light: "#ffffffff" }
  });
  const qrImage = await loadImage(qrDataUrl);
  const footerY = CANVAS_HEIGHT - 28;
  const qrCardHeight = 176;
  const qrCard = { x: CONTENT_X, y: Math.max(summaryCard.y + summaryCard.height + 36, footerY - 48 - qrCardHeight), width: CONTENT_WIDTH, height: qrCardHeight };
  ...
  const qrNoteLayout = fitWrappedText(context, copy.shareImageQrNote, 650, { fontSizes: [28, 26, 24], lineHeightMultiplier: 1.28, maxLines: 3, weight: 500 });
...
function drawRangeBadge(context: CanvasRenderingContext2D, language: H5Language, label: string, background: string, textColor: string) {
...
function drawMetricCard(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, label: string, value: string, note: string, accent: string) {
...
function drawPanel(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, radius: number) {
...
function fitWrappedText(context: CanvasRenderingContext2D, text: string, maxWidth: number, options: { fontSizes: number[]; lineHeightMultiplier: number; maxLines: number; weight: number; }): FittedTextLayout {
  ...
  options.fontSizes.forEach((fontSize) => {
    ...
    if (lines.length <= options.maxLines) {
      fitted = { lines, fontSize, lineHeight: Math.round(fontSize * options.lineHeightMultiplier) };
    }
  });
  if (fitted) return fitted;
  const fontSize = options.fontSizes[options.fontSizes.length - 1];
  setFont(context, options.weight, fontSize);
  return { lines: fitLines(wrapLines(context, text, maxWidth), options.maxLines), fontSize, lineHeight: Math.round(fontSize * options.lineHeightMultiplier) };
}
